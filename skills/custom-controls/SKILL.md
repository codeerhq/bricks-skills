...
1. **`bricks/get-element-schema` MCP tool**: returns the full control tree for any element. Use when you need exact argument shape for a specific control.
...
You do **not** emit `<style>` in `render()`. Bricks collects every `css` entry across all controls, generates a stylesheet, and loads it (inline or file, see `performance` skill).
