...
Full loop-tag cheatsheet in the `query-loops` skill.
...
`{echo:function_name(arg1, arg2)}` calls a PHP function at render time. Full threat model in the `custom-code` skill. Short version:
...
See the `custom-dynamic-data-providers` skill for the full pattern.
...
- Use `{echo:...}` as a general-purpose function caller without seeing the `custom-code` skill first: echo is a security surface.
