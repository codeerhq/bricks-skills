...
2. **Load the task skill before the first write.** If you are about to author a design system, load **bricks-design-systems** or **bricks-seed-design-system**. If you are creating, updating, extracting, instancing, or deleting components, load **bricks-components**. If you are setting template conditions, load **bricks-templates-conditions**. If you are writing element `_conditions`, load **bricks-element-conditions**. If you are writing element `_interactions`, load **bricks-interactions**. If you are creating a mega menu, load **bricks-mega-menus**. If you are creating a new visual section, page, or template shell, load **bricks-html-css-to-bricks**. Do this before writing data, not after the first error.
...
6. **Do not guess element settings.** Before writing an unfamiliar element, a complex control, or any media/query/form/interaction/condition setting, call `mcp-adapter-execute-ability` with `ability_name: "bricks/get-element-schema"` and `parameters: { "elementName": "<name>" }`. This ability is dispatcher-only on the default Bricks MCP server. Then use the `bricks-element-schemas` skill for the control value schema when the runtime payload only says `type: "image"`, `type: "query"`, `type: "typography"`, `type: "repeater"`, etc.
...
9. **Post revisions are the only automatic undo.** Bricks post revisions are snapshotted automatically on element writes. Global-data writes (classes, variables, theme styles, components, breakpoints, pseudo-classes) are not revision-tracked. Before destructive global-data changes, load **bricks-import-export**, call `bricks/list-transfer-items`, and export the affected supported items with `bricks/export-transfer-package`. To restore, inspect that exact package with `bricks/inspect-transfer-package`, then pass its returned `zipHash` to `bricks/import-transfer-package` as `expectedZipHash` together with explicit item IDs. Replacements require explicit user intent and `allowOverwrite: true`.
...
3. Load the task-specific skill before the first write: **bricks-design-systems** / **bricks-seed-design-system**, **bricks-templates-conditions**, **bricks-element-conditions**, **bricks-interactions**, **bricks-mega-menus**, **bricks-html-css-to-bricks**, **bricks-query-loops**, **bricks-query-filters**, **bricks-forms**, or **bricks-components**.
...
6. For new static visual shells, write HTML/CSS and run `bricks/convert-html-css-to-bricks-data` before saving. For Bricks-native behavior, fetch the runtime element schema and consult `bricks-element-schemas` for complex control value shapes.
...
9. If a browser or screenshot tool is available, load **bricks-browser-verify** and visually verify important pages after broad element/template writes.
...
- **Template conditions accept arrays, not scalar strings.** Use `postType: ["<post-type-slug>"]`, `archiveType: ["postType"]`, and `archivePostTypes: ["<post-type-slug>"]`. Load **bricks-templates-conditions** before writing template conditions.
...
- Authoring a design system: **bricks-design-systems**, **bricks-seed-design-system**
- New visual sections/pages/templates from HTML/CSS: **bricks-html-css-to-bricks**
- Template condition setup/scoring: **bricks-templates-conditions**
- Element display conditions: **bricks-element-conditions**
- Element interactions and popup/open/scroll behavior: **bricks-interactions**
- Mega menus and header navigation panels: **bricks-mega-menus**
- Auditing the site: **bricks-audit-design-system**, **bricks-site-audit**
- Planning a change from a user brief: **bricks-plan-from-brief**
- Enforcing naming: **bricks-naming-conventions**
- Feature-specific gotchas: **bricks-query-loops**, **bricks-components**, **bricks-custom-code**
- Exact element/control schemas: **bricks-element-schemas**
- Uploading and wiring images/media: **bricks-media-assets**
- Browser/screenshot verification: **bricks-browser-verify**
