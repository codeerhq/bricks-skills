...
6. **Do not guess element settings.** Before writing an unfamiliar element, a complex control, or any media/query/form/interaction/condition setting, call `bricks-get-element-schema` or dispatch `ability_name: "bricks/get-element-schema"`. Then use the `bricks-element-schemas` skill for the control value schema when the runtime payload only says `type: "image"`, `type: "query"`, `type: "typography"`, `type: "repeater"`, etc.
