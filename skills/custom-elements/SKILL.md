...
See the `custom-controls` skill for the complete list. The big ones: `text, textarea, number, select, checkbox, color, typography, background, border, box-shadow, spacing, dimensions, icon, image, repeater, code, query, query-list, link`.
...
See `nestable-elements` skill for the full contract.
...
- `custom-controls`: control type reference and patterns.
- `custom-code`: where element PHP fits among the 7 extension points.
- `hooks-reference`: hooks like `bricks/element/render_attributes` for post-render tweaks.
- `child-theme-patterns`: where to put element files in a child theme.
