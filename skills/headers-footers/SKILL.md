...
Bricks finds the active header/footer template via `find_template_id( 'header' | 'footer' )`: same scoring as content templates (see `templates-conditions` skill). The highest-scoring matching template wins.
...
2. Are there multiple header/footer templates with overlapping conditions? Check scoring (`templates-conditions` skill).
