---
name: ai-tab
description: "Use when checking Bricks' Settings > AI tab: the master Bricks abilities toggle, per-ability disable list, direct-tool fast path, `bricks-list-ability-status`, and error codes when an ability is off."
---
...
# Bricks: AI settings tab

Bricks exposes its abilities to MCP clients through `Bricks > Settings > AI`. This tab is the admin control panel for Bricks abilities. MCP clients cannot change these settings. The tab affects which Bricks abilities are callable.
...
If `enabled: false`, a site admin either disabled the ability or has not opted into a default-off group. You cannot route around it. The fix is for a human to enable it under `Bricks > Settings > AI`.
...
- **Master toggle off**: no dedicated error code. With Bricks MCP disabled, no `bricks/*` abilities register at all. Tell the user to enable `Bricks > Settings > AI > Enable Bricks MCP`.
- **`bricks_setting_excluded`** / **`bricks_setting_unknown`**: these come from the settings registry, not the AI tab. See the `bricks-settings` skill.

## How the tab interacts with call-time checks

The AI tab is an exposure deny-list, not a role editor:
...
#  Re-enable it under Bricks > Settings > AI."
```
...
- Don't bypass the AI tab by editing `bricks_mcp_settings` directly. The UI is the contract.
